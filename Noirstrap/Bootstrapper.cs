...
using Noirstrap.AppData;
using Noirstrap.Integrations;
using Noirstrap.Integrations.AssetProxy;
using Noirstrap.Models.APIs;
using Noirstrap.Models.APIs.RoValra;
using Noirstrap.RobloxInterfaces;
using Noirstrap.UI.Elements.Bootstrapper.Base;
...
namespace Noirstrap
{
    public class Bootstrapper
    {
        ...
        // ===== ExploitStrap port: Versions Manager + LIVE channel lock =====
        ...
        private void FetchCurrentChannel()
        {
            ...
            if (_channelFetched)
                return;
            // ExploitStrap behavior: lock to LIVE (production). Keeps executors from
            // breaking on surprise Canary/Integration reroutes. Noirstrap channel
            // setting is preserved in Settings.Channel but launch always uses LIVE
            // when a version profile is active; otherwise fall back to configured channel.
            var active = GetActiveProfileForBootstrap();
            if (active != null || App.Settings.Prop.ShowLiveChannelToast)
            {
                Deployment.Channel = Deployment.DefaultChannel;
                App.Logger.WriteLine(LOG_IDENT, $"Channel forced to {Deployment.DefaultChannel} (LIVE lock)");
            }
        ...
        private void UpdateChannelRegistry()
        {
            ...
            for (int attempt = 1; attempt <= 2; attempt++)
            {
                try
                {
                    ...
                    App.Logger.WriteLine(LOG_IDENT, $"Verification MISMATCH on attempt {attempt}: read back '{readBack}'");
            ...
            try { Utility.LiveChannelToast.ShowChannelLockFailed(lastReason); } catch { }
        }
        ...
        public async Task Run()
        {
            ...
            if (!App.LaunchSettings.NoLaunchFlag.Active && !_cancelTokenSource.IsCancellationRequested)
            {
                ...
                try { Utility.LiveChannelToast.Show(); } catch { }
            }
        ...
        private async Task GetLatestVersionInfo()
        {
            ...
            // ===== ExploitStrap port: Versions Manager resolution =====
            // Refresh executor-tracked active profile (3s budget, never blocks launch)
            if (_launchMode == LaunchMode.Player && !App.LaunchSettings.VersionFlag.Active)
            {
                try { await Utility.ExecutorProfileRefresher.RefreshActiveAsync(TimeSpan.FromSeconds(3)); }
                catch { }
            }

            var launchProfile = GetActiveProfileForBootstrap();
            if (!App.LaunchSettings.VersionFlag.Active
                && launchProfile != null
                && !string.IsNullOrEmpty(launchProfile.VersionGuid)
                && Utility.VersionGuidValidator.IsWellFormed(launchProfile.VersionGuid))
            {
                App.Logger.WriteLine(LOG_IDENT, $"Following Versions Manager profile '{launchProfile.Name}' ({launchProfile.VersionGuid})");
                App.LaunchSettings.VersionFlag.Active = true;
                App.LaunchSettings.VersionFlag.Data = launchProfile.VersionGuid;
            }
            else if (!App.LaunchSettings.VersionFlag.Active
                && App.Settings.Prop.UseCustomVersion
                && Utility.VersionGuidValidator.IsWellFormed(App.Settings.Prop.CustomVersionGuid))
            {
                App.Logger.WriteLine(LOG_IDENT, $"Following downgrade pin {App.Settings.Prop.CustomVersionGuid}");
                App.LaunchSettings.VersionFlag.Active = true;
                App.LaunchSettings.VersionFlag.Data = App.Settings.Prop.CustomVersionGuid;
            }
            ...
            {
                var activeForDir = GetActiveProfileForBootstrap();
                if (activeForDir != null && !string.IsNullOrEmpty(_latestVersionGuid))
                {
                    try
                    {
                        Directory.CreateDirectory(Paths.Versions);
                        _latestVersionDirectory = Utility.VersionProfileLayout.EnsureActive(activeForDir, _latestVersionGuid);
                        // stamp installed hash for future launches
                        if (activeForDir.InstalledVersionGuid != _latestVersionGuid)
                        {
                            activeForDir.InstalledVersionGuid = _latestVersionGuid;
            ...
            // LIVE lock: enforce empty channel key so Roblox launches on production
            try { UpdateChannelRegistry(); } catch { }
        ...
        private async Task<bool> CheckForUpdates()
        {
            ...
            {
                App.Logger.WriteLine(LOG_IDENT, $"More than one Noirstrap instance running, aborting update check");
                return false;
            }
            ...
            SetStatus(Strings.Bootstrapper_Status_UpgradingNoirstrap);

            try
            {
#if DEBUG_UPDATER
                string downloadLocation = Path.Combine(Paths.TempUpdates, "Noirstrap.exe");
        ...
        public static void CleanupVersionsFolder()
        {
            ...
            foreach (string dir in Directory.GetDirectories(Paths.Versions))
            {
                ...
                // ExploitStrap port: never delete parked profile installs or pinned hashes
                if (dirName.StartsWith("profile-", StringComparison.OrdinalIgnoreCase))
                    continue;
                if (dirName.StartsWith(".stale-", StringComparison.OrdinalIgnoreCase))
                    continue;
                try
                {
                    var pinned = App.Settings.Prop.VersionProfiles
                        .Where(p => !string.IsNullOrEmpty(p.VersionGuid) || !string.IsNullOrEmpty(p.InstalledVersionGuid))
                        .SelectMany(p => new[] { p.VersionGuid, p.InstalledVersionGuid })
                        .Where(g => !string.IsNullOrEmpty(g))
                        .ToHashSet(StringComparer.OrdinalIgnoreCase);
                    if (pinned.Contains(dirName))
                        continue;
                    if (!string.IsNullOrEmpty(App.Settings.Prop.CustomVersionGuid)
                        && string.Equals(dirName, App.Settings.Prop.CustomVersionGuid, StringComparison.OrdinalIgnoreCase))
                        continue;
                }
                catch { }
                ...
                catch (Exception ex)
                {
                    ...
                    if (ex.GetType() == typeof(ChecksumFailedException))
                    {
                        Frontend.ShowConnectivityDialog(
                            ...
                            String.Format(Strings.Dialog_Connectivity_UnableToDownloadReason, "[https://github.com/noirstraplabs/noirstrap/wiki/Noirstrap-is-unable-to-download-Roblox](https://github.com/noirstraplabs/noirstrap/wiki/Noirstrap-is-unable-to-download-Roblox)"),
