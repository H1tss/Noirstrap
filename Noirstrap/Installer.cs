...
namespace Noirstrap
{
    internal class Installer
    {
        ...
        public string NoirstrapInstallDirectory = Path.Combine(Paths.LocalAppData, "Noirstrap"); // default directory for noirstrap
        ...
        public bool ImportSettings = Directory.Exists(Path.Combine(Paths.LocalAppData, "Noirstrap")); // if noirstrap isnt detected this will be set to false
        ...
        public void DoInstall()
        {
            ...
            // only register player, for the scenario where the user installs noirstrap, closes it,
        ...
        private bool ValidateLocation()
        {
            ...
            if (InstallLocation.Contains("Local\\Noirstrap"))
                return false;
        ...
        public static void HandleUpgrade()
        {
            ...
            if (existingVer is not null)
            {
                ...
                if (Utilities.CompareVersions(existingVer, "2.8.0") == VersionComparison.LessThan)
                {
                    ...
                    string oldStartPath = Path.Combine(Paths.WindowsStartMenu, "Noirstrap");
                    ...
                    Registry.CurrentUser.DeleteSubKeyTree("Software\\Noirstrap", false);
...
#pragma warning restore CS0162 // Unreachable code detected
        ...
        public void ImportSettingsFromNoirstrap()
        {
            const string LOG_IDENT = "Installer::ImportSettings";

            if (!Directory.Exists(NoirstrapInstallDirectory))
            {
                Frontend.ShowMessageBox(Strings.Installer_InstallationNotFound, MessageBoxImage.Exclamation);
                return;
            } // noirstrap default directory is not present

            foreach (string FileName in FilesForImporting)
            {
                string Source = Path.Combine(NoirstrapInstallDirectory, FileName);
                ...
                if (IsDirectory)
                {
                    ...
                    // we could use Directory.Move but that deletes the directory from noirstrap folder
