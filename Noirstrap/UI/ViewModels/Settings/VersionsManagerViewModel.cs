...
namespace Noirstrap.UI.ViewModels.Settings
{
    ...
    public class VersionsManagerViewModel : NotifyPropertyChangedViewModel
    {
        ...
        private void DeleteProfile(string? id)
        {
            ...
            Noirstrap.Utility.FastFlagProfiles.Delete(id);
        ...
        private void SetAsInstallTarget(string? id)
        {
            ...
            string resolved = Noirstrap.Utility.VersionProfileLayout.EnsureActive(profile, profile.VersionGuid);

            if (Noirstrap.Utility.VersionProfileLayout.IsInstallTarget(profile.Id))
            {
    ...
    public class VersionProfileTile : INotifyPropertyChanged
    {
        ...
        private static ImageSource? LoadRobloxIcon()
        {
            try
            {
                using var stream = Noirstrap.Resource.GetStream("Icon2022.ico");
        ...
        internal static bool ResolveIsInstallTarget(string versionGuid, string profileId)
            => Noirstrap.Utility.VersionProfileLayout.IsInstallTarget(profileId);
