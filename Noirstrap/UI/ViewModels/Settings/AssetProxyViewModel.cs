        ...
        public string HostsStateText
        {
            get
            {
                if (HasStaleHostsEntries)
                    return "Roblox's asset hostnames are still pointed at Noirstrap, but nothing is serving them - Roblox will not load assets until this is repaired";
        ...
        public string StatusText
        {
            get
            {
                ...
                if (_hostStatus.RobloxRunning)
                    return $"Intercepting on port {AssetProxyManager.OriginPort} - Roblox is open but started outside Noirstrap, so it is not being intercepted";

                return $"Intercepting on port {AssetProxyManager.OriginPort} - waiting for a game launched from Noirstrap";
            }
        }
