...
namespace Noirstrap.Integrations.AssetProxy
{
    public class AssetProxyServer : IDisposable
    {
        ...
        private const string ControlPrefix = "/__noirstrap/";
        ...
        {
            using var rsa = RSA.Create(2048);
            var request = new CertificateRequest("CN=Noirstrap Local CA, O=Noirstrap", rsa, HashAlgorithmName.SHA256, RSASignaturePadding.Pkcs1);
        ...
        private async Task HandleControlRequestAsync(NetworkStream stream, string requestLine)
        {
            ...
            {
                App.Logger.WriteLine(LOG_IDENT, "Stop asked for by another Noirstrap process");
                ...
                if (rule is not null)
                {
                    switch (rule.Action)
                    {
                        case AssetProxyAction.Remove:
                            ...
                            await SendSimpleResponseAsync(clientStream, version, 404, "Not Found", "text/plain", Encoding.UTF8.GetBytes("Asset removed by Noirstrap"));
                            return true;
        ...
        private async Task<bool> ForwardRequestAsync(Stream clientStream, string host, string method, string target, Dictionary<string, string> headers, byte[] body, string version, long? assetId, Uri uri, bool useTls, bool cacheable, bool clientKeepAlive)
        {
            ...
            try
            {
                ...
                {
                    await SendSimpleResponseAsync(clientStream, version, 502, "Bad Gateway", "text/plain", Encoding.UTF8.GetBytes("Noirstrap asset proxy: upstream sent nothing"));
                    return true;
                }
            ...
            catch (Exception ex)
            {
                ...
                if (!responseStarted)
                {
                    try
                    {
                        await SendSimpleResponseAsync(clientStream, version, 502, "Bad Gateway", "text/plain", Encoding.UTF8.GetBytes("Noirstrap asset proxy: upstream request failed"));
                    }
                    catch
                    {
                    }
