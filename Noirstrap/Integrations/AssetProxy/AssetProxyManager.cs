...
namespace Noirstrap.Integrations.AssetProxy
{
    public static class AssetProxyManager
    {
        ...
        public static AssetProxyStatus? GetHostStatus(int port = DefaultPort, int timeoutMs = 750)
        {
            try
            {
                ...
                string json = ControlClient
                    .GetStringAsync($"http://127.0.0.1:{port}/__noirstrap/status", cts.Token)
                    .GetAwaiter()
                    .GetResult();
        ...
        private static bool TrySendControl(string command, int port = DefaultPort, int timeoutMs = 400)
        {
            try
            {
                ...
                using var response = ControlClient
                    .GetAsync($"http://127.0.0.1:{port}/__noirstrap/{command}", cts.Token)
                    .GetAwaiter()
                    .GetResult();
        ...
        public static string CaCertificatePath => Path.Combine(AssetProxyDirectory, "noirstrap-ca.pem");
        ...
        private const string CaMarker = "# Noirstrap Local CA - do not remove";
        ...
        private static void PatchRobloxCaCertificates(X509Certificate2 caCertificate)
        {
            ...
            foreach (string versionDirectory in versionDirectories.Distinct())
            {
                ...
                try
                {
                    ...
                    int removed = StripNoirstrapCertificates(ref content);

                    File.WriteAllText(cacertPath, content.TrimEnd('\r', '\n') + "\n\n" + block + "\n");

                    App.Logger.WriteLine(LOG_IDENT, removed > 0
                        ? $"Replaced {removed} Noirstrap CA entr{(removed == 1 ? "y" : "ies")} in {cacertPath}"
                        : $"Patched {cacertPath}");
                }
                catch (Exception ex)
                {
        ...
        private static int StripNoirstrapCertificates(ref string content)
        {
            ...
            while (true)
            {
                ...
                if (IsNoirstrapCertificate(blockText))
                    removed++;
                else
        ...
        private static bool IsNoirstrapCertificate(string blockText)
        {
            const string begin = "-----BEGIN CERTIFICATE-----";
            const string end = "-----END CERTIFICATE-----";
            ...
            try
            {
                ...
                return certificate.Subject.Contains("Noirstrap Local CA", StringComparison.OrdinalIgnoreCase);
            }
            catch
            {
                return false;
